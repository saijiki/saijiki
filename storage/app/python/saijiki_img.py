...
# コマンドライン引数
KEYWORD = sys.argv[1] # 生成キーワード
TARGET_SENTENCE_1 = sys.argv[2] # 初句
TARGET_SENTENCE_2 = sys.argv[3] # 二句
TARGET_SENTENCE_3 = sys.argv[4] # 結句
...
# キーワード強調
sentence_1_colors = ['black'] * len(TARGET_SENTENCE_1)
sentence_2_colors = ['black'] * len(TARGET_SENTENCE_2)
sentence_3_colors = ['black'] * len(TARGET_SENTENCE_3)

# 句の中からキーワードを検索　→　文字色を変更
for i in range(TARGET_SENTENCE_1.find(KEYWORD), len(KEYWORD)):
    sentence_1_colors[i] = 'red'
...
for i, char in enumerate(TARGET_SENTENCE_1):
    draw.text((sentence_1_x + (FONT_SIZE - sentence_1_widths[i]) / 2 - 1, sentence_1_y + sum(sentence_1_heights[:i]) - 1), char, fill=sentence_1_colors[i], font=font)
    draw.text((sentence_1_x + (FONT_SIZE - sentence_1_widths[i]) / 2    , sentence_1_y + sum(sentence_1_heights[:i])    ), char, fill=sentence_1_colors[i], font=font)
    draw.text((sentence_1_x + (FONT_SIZE - sentence_1_widths[i]) / 2 + 1, sentence_1_y + sum(sentence_1_heights[:i]) + 1), char, fill=sentence_1_colors[i], font=font)

# 二句を描画 (三重描画で太字にする)
for i, char in enumerate(TARGET_SENTENCE_2):
    draw.text((sentence_2_x + (FONT_SIZE - sentence_2_widths[i]) / 2 - 1, sentence_2_y + sum(sentence_2_heights[:i]) - 1), char, fill=sentence_2_colors[i], font=font)
    draw.text((sentence_2_x + (FONT_SIZE - sentence_2_widths[i]) / 2    , sentence_2_y + sum(sentence_2_heights[:i])    ), char, fill=sentence_2_colors[i], font=font)
    draw.text((sentence_2_x + (FONT_SIZE - sentence_2_widths[i]) / 2 + 1, sentence_2_y + sum(sentence_2_heights[:i]) + 1), char, fill=sentence_2_colors[i], font=font)

# 結句を描画 (三重描画で太字にする)
for i, char in enumerate(TARGET_SENTENCE_3):
    draw.text((sentence_3_x + (FONT_SIZE - sentence_3_widths[i]) / 2 - 1, sentence_3_y + sum(sentence_3_heights[:i]) - 1), char, fill=sentence_3_colors[i], font=font)
    draw.text((sentence_3_x + (FONT_SIZE - sentence_3_widths[i]) / 2    , sentence_3_y + sum(sentence_3_heights[:i])    ), char, fill=sentence_3_colors[i], font=font)
    draw.text((sentence_3_x + (FONT_SIZE - sentence_3_widths[i]) / 2 + 1, sentence_3_y + sum(sentence_3_heights[:i]) + 1), char, fill=sentence_3_colors[i], font=font)
